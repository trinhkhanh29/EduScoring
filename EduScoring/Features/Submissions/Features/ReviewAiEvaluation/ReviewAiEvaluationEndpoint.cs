...
public static class ReviewAiEvaluationEndpoint
{
    public static void MapReviewAiEvaluationEndpoint(this IEndpointRouteBuilder app)
    {
        app.MapPost("/api/submissions/{id:guid}/review-ai", async (
            Guid id,
            ClaimsPrincipal user,
            ReviewAiEvaluationCommandHandler handler) =>
        {
            var tag = $"[ReviewAiEvaluation | EntityId={id}]";
            ...
            var command = new ReviewAiEvaluationCommand(id, userId, isAdmin);
